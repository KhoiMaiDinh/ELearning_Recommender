pipeline {
    ...
    environment {
        IMAGE_TAG = 'latest'
        IMAGE_NAME = 'eventhub.recommender'
    ...
    stages {       
        stage('Build') {
            steps {
                script {
                    sh "cp /home/eventhub/eventhub-recommender-system/.env ./.env"
    ...
    post {
        success {
            script {
                ...
                def jsonText = "[🔥SUCCESS][eventhub-recommender-system] Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} success🔥🔥🔥! For more info: ${env.BUILD_URL}"
        ...
        failure {
            script {
                def jsonText = "[💀FAILED][eventhub-recommender-system] Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} failed😭😭😭! For more info: ${env.BUILD_URL}"
                def payload = groovy.json.JsonOutput.toJson([
                    chat_id: "1934277483",
                    text: jsonText,
                    disable_notification: false
